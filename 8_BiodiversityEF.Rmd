...
Generally holds, BUT (interestingly) more species usually means less species-level stability
...
**Logic:** Invaders must find resources to survive and grow, but theory predicts that levels of unconsumed resources decline as diversity increases (and is supported by B-EF experiments).
...
## B-EF and global change: Response vs effect types?
...
- Fynbos is physiognomally and functionally similar across huge variation in climate, soils etc.
- This is largely because most communities are dominated by the same 10 or so lineages.
- Species look similar, and have similar effects on the environment, but must be responding differently to occur in such different environments
...
```{r echo = F, fig.align = 'right', out.width = '65%'}
...
## B-EF and global change: Regime Shifts

Alternative vegetation states can exist on the same abiotic template. They often contrast dramatically in various ecosystem functions (productivity, decomposition, habitat, etc).

They occur naturally, e.g.
- Open (fynbos, grassland, savanna) vs closed (forest) ecosystems

Or can be created unnaturally, e.g.
- Fynbos/grassland vs pine invasion
- Expansion of closed into open ecosystems (or vice versa)
...
```{r echo = F, fig.align = 'right', out.width = '110%'}
knitr::include_graphics("images/slingsby2020_ICb.png")
```

.footnote[Slingsby et al. 2020]
]
