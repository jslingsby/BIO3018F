...
site <- read_xlsx("prac/flammability/flammabilitytraits2024.xlsx","SiteData2023")
```
...
site %>%
  ...
  rowwise() %>%
  mutate(Dung = sum(c(Dung_Eland, Dung_Bontebok, Dung_Ostrich, Dung_Other))) %>%
  pivot_longer(cols = c("CanopyCover", "Dung", "SoilPH", "PercentBareSoil")) %>%
...
hmm <- classification(data$Taxon, db = "ncbi")
...
.pull-left[
```{r, eval = FALSE}
tclass <- cutree(as.hclust(ape::multi2di(tree$phylo)), k = 6)

data <- left_join(data, data.frame(TaxonClass = as.factor(tclass), Taxon = names(tclass)))
...
```{r, fig.align = 'center', out.width = '50%', fig.asp=1.25}
# Unweighted
...
### Plant traits: continuous by TaxonClass

```{r, fig.align = 'center', out.width = '90%', fig.asp=1.5, eval = FALSE}
data %>% ggduo("TaxonClass", cont_trts)
```
]

.pull-right[

```{r, fig.align = 'center', out.width = '90%', fig.asp=1.5, echo = FALSE}
data %>% ggduo("TaxonClass", cont_trts)
```
]
